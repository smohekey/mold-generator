...
fn run() -> Result<(), Box<dyn Error>> {
    ...
    let input = args.next().ok_or_else(|| usage("missing input STL"))?;
    let output = args.next().ok_or_else(|| usage("missing output STL"))?;
    let margin = match args.next() {
        Some(value) => value
            .parse::<f64>()
            .map_err(|_| usage("margin must be a number"))?,
        None => 10.0,
    };
    ...
    if !margin.is_finite() || margin <= 0.0 {
        return Err(usage("margin must be finite and greater than zero").into());
    }
