use crate::{
    WingError, WingSpec, WingSurface, cross_array, interpolate_station_extended, normalize_array,
    subtract, surface_point,
};
...
/// Returns a point and outward normal on an analytic wing surface, extending
/// the root and tip stations when `span` lies just beyond the modeled wing.
pub fn wing_surface_frame(
    ...
    let station = interpolate_station_extended(wing, span)?;
    ...
    let (span_before, span_after) = if span < first {
        (span, (span + span_step).min(first))
    } else if span > last {
        ((span - span_step).max(last), span)
    } else {
        ((span - span_step).max(first), (span + span_step).min(last))
    };
    let before = interpolate_station_extended(wing, span_before)?;
    let after = interpolate_station_extended(wing, span_after)?;
...
mod tests {
    ...
    fn frame_before_root_matches_the_extended_root_profile() {
        ...
        assert!((frame.point[1] + 4.0).abs() < 1.0e-9);
        assert!(frame.outward_normal[2] > 0.0);
    }
}
