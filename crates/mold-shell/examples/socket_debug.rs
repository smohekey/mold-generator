...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, insert) in [("a", &insert_a), ("b", &insert_b)] {
        let lower = lower_flange.0.intersection(&insert.0).volume();
        let upper = upper_flange.0.intersection(&insert.0).volume();
        println!("insert {name}: raw flange intersection lower={lower:.6} upper={upper:.6}");
    }

    let kernel = ManifoldKernel;
    let none: [&ManifoldSolid; 0] = [];
    let mold = generate_sectioned_shell_mold_with_parting(
        ...
        PartingRegions {
            ...
            negative_sockets: &none,
            positive_sockets: &none,
            negative_webbing_exclusions: &none,
            positive_webbing_exclusions: &none,
        },
        Axis::Y,
...
fn diamond_prism(
    ...
    let footprint = [
        transform_station(
            &center,
            flange_center_x(&center, side) - chord_half_width,
            0.0,
        ),
        transform_station(&inboard, flange_center_x(&inboard, side), 0.0),
        transform_station(
            &center,
            flange_center_x(&center, side) + chord_half_width,
            0.0,
        ),
        transform_station(&outboard, flange_center_x(&outboard, side), 0.0),
    ];
...
fn flange_normal(
    ...
    let mut normal = normalize(cross(sub(chord_b, chord_a), sub(span_b, span_a)))?;
    if normal[2] < 0.0 {
