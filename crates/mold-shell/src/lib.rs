...
/// Optional flange and registration solids allow non-planar parting surfaces
/// to carry their own printable mating geometry.
pub struct PartingRegions<'a, S> {
    pub negative: &'a S,
    pub positive: &'a S,
    pub negative_flange: Option<&'a S>,
    pub positive_flange: Option<&'a S>,
    /// Solids added to the negative half as male registration keys.
    pub negative_keys: &'a [&'a S],
    /// Matching clearance solids subtracted from the positive half.
...
where K: SolidKernel {
    ...
    for key in parting.negative_keys { negative_skin = kernel.union(&negative_skin, key)?; }
...
where K: SolidKernel {
    ...
    for &(section_min, section_max) in sections {
        let mut clip = bounds; set_min(&mut clip, section_axis, section_min); set_max(&mut clip, section_axis, section_max);
...
fn generate_half<K>(kernel: &K, part: &K::Solid, skin: &K::Solid, part_bounds: Bounds3, half_bounds: Bounds3, split_axis: Axis, section_axis: Axis, split: f64, section_ranges: &[(f64, f64)], settings: ShellSettings, half: Half) -> Result<Vec<K::Solid>, K::Error>
where K: SolidKernel {
    let mut pieces = Vec::with_capacity(section_ranges.len());
    for &(section_min, section_max) in section_ranges {
        let mut clip = half_bounds; set_min(&mut clip, section_axis, section_min); set_max(&mut clip, section_axis, section_max);
        let mut piece = kernel.intersection(skin, &kernel.cuboid(clip)?)?;
        piece = union_web(kernel, part, piece, split_web_bounds(part_bounds, clip, split_axis, section_axis, split, settings, half))?;
        piece = union_web(kernel, part, piece, section_web_bounds(part_bounds, clip, split_axis, section_axis, section_min, settings, WebSide::Min))?;
        piece = union_web(kernel, part, piece, section_web_bounds(part_bounds, clip, split_axis, section_axis, section_max, settings, WebSide::Max))?;
        pieces.push(piece);
    }
    Ok(pieces)
}
